...
public abstract class Enemy : MonoBehaviour
{
    ...
    [SerializeField] protected LayerMask _attackedMask;
    ...
    {
           Collider[] col = Physics.OverlapSphere(gameObject.transform.position,_patrolRadius,_attackedMask);
           if(col.Length <= 0)   return;

             Player = col[0].GetComponent<Player>();
             LookRotationTarget(Player.transform);
            if(_isAttack) 
            {
                _agent.isStopped = false;
                MoveAgent(Player.transform);
            } 
            else 
            {
